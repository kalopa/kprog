...
intel_parse_line(char *linep)
{
	...
	if (*linep++ != ':') {
		fprintf(stderr, "avrprog: bad Intel HEX format in file - missing header.\n");
		exit(1);
	}
	...
	csum &= 0xff;
	if ((csum & 0xff) != 0x00) {
		fprintf(stderr, "avrprog: invalid checksum in Intel HEX file.\n");
		exit(1);
	}
	return(type);
}
...
get_hex_bytes(char *cp, int len)
{
	...
	while (len-- > 0) {
		ch = *cp++;
		if (!isxdigit(ch)) {
			fprintf(stderr, "avrprog: Bad hexadecimal data in file.\n");
			exit(1);
		}
		if (ch >= '0' && ch <= '9') {
			ch -= '0';
